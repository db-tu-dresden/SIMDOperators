...
/**
 * @file sort_indirect_inplace.hpp
 * @brief
 */

#ifndef SIMDOPS_INCLUDE_ALGORITHMS_DBOPS_SORT_SORT_INDIRECT_INPLACE_HPP
#define SIMDOPS_INCLUDE_ALGORITHMS_DBOPS_SORT_SORT_INDIRECT_INPLACE_HPP
...
namespace tuddbs {
  ...
  class SingleColumnSortIndirectInplace {
   ...
   private:
    ...
    constexpr static size_t idx_arr_len = sizeof(typename I::base_type) / sizeof(typename S::base_type);

    template <class S, class I>
    constexpr static size_t bits_per_idx_register =
...
#pragma unroll(idx_arr_len<SimdStyle, IndexStyle>)
      ...
      if ((left_w - left_start) < (4 * SimdStyle::vector_element_count())) {
        ...
        partition<SimdStyle, IndexStyle>(data, indexes, left_start, left_w, pivot_ls);
      }
      ...
      if ((right_start - pivot_r_w) < (4 * SimdStyle::vector_element_count())) {
        ...
        partition<SimdStyle, IndexStyle>(data, indexes, pivot_r_w, right_start, pivot_rs);
      }
    }
  };
};  // namespace tuddbs
