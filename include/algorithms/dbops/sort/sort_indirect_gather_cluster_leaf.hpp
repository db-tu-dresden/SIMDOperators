...
#ifndef SIMDOPS_INCLUDE_ALGORITHMS_DBOPS_SORT_SORT_INDIRECT_GATHER_CLUSTER_LEAF_HPP
#define SIMDOPS_INCLUDE_ALGORITHMS_DBOPS_SORT_SORT_INDIRECT_GATHER_CLUSTER_LEAF_HPP
...
namespace tuddbs {
  ...
  class LeafClusteringSingleColumnSortIndirectGather {
   ...
   private:
    ...
    __attribute__((always_inline)) inline auto compare(const typename SimdStyle::register_type val_reg,
      ...
      // std::cout << "Pivot: ";
      // print_reg<SimdStyle>(pivot_reg);
      // std::cout << "Data: ";
      // print_reg<SimdStyle>(val_reg);
      // std::cout << "Comparison with " << tsl::type_name<SimdStyle>() << std::endl;
    ...
    void partition(std::deque<Cluster>& clusters, T const* __restrict__ data, U* __restrict__ indexes, size_t left,
                   size_t right, T pivot) {
      ...
      if ((left_w - left_start) < (4 * IndexStyle::vector_element_count())) {
        ...
        detect_cluster(clusters, data, indexes, left_start, left_w);
      } else {
        const auto pivot_ls = get_pivot_indirect(data, indexes, left_start, left_w - 1);
        partition<SimdStyle, IndexStyle>(clusters, data, indexes, left_start, left_w, pivot_ls);
      }
      ...
      if ((right_start - pivot_r_w) < (4 * IndexStyle::vector_element_count())) {
        ...
        detect_cluster(clusters, data, indexes, left_w, right_start);
      } else {
        const auto pivot_rs = get_pivot_indirect(data, indexes, pivot_r_w, right_start - 1);
        detect_cluster(clusters, data, indexes, left_w, pivot_r_w);
        partition<SimdStyle, IndexStyle>(clusters, data, indexes, pivot_r_w, right_start, pivot_rs);
      }
    }
  };
}  // namespace tuddbs
