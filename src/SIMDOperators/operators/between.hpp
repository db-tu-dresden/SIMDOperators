 ...
 */
// ------------------------------------------------------------------- //
#ifndef SRC_SIMDOPERATORS_OPERATORS_BETWEEN_HPP
#define SRC_SIMDOPERATORS_OPERATORS_BETWEEN_HPP
...
namespace tuddbs{
    ...
    class between {
        ...
        class batch {
            // to make it accessable by between class
            friend class between<ProcessingStyle>;
            ...
            static size_t apply(base_type * result, const base_type * column, base_type predicate_lower, base_type predicate_upper, const size_t& vector_count, size_t start_index = 0){
                if(vector_count == 0){
                    return 0;
                }
                /// [predicate, ...]
                reg_t const predicate_vector_lower = tsl::set1<batchps>(predicate_lower);
                reg_t const predicate_vector_upper = tsl::set1<batchps>(predicate_upper);
                /// [element_count, ...]
                ...
                for (size_t i = 0; i < vector_count; ++i) {
                    ...
                    mask_t mask = tsl::between_inclusive<batchps, tsl::workaround>(data_vector, predicate_vector_lower, predicate_vector_upper);
    ...
    public:
        ...
        DBTUD_CXX_ATTRIBUTE_FORCE_INLINE
        static col_ptr apply(const_col_ptr column, const base_type& predicate_lower, const base_type& predicate_upper){
...
#endif //SRC_SIMDOPERATORS_OPERATORS_BETWEEN_HPP
