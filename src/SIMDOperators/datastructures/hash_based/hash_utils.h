...
namespace vectorlib {
   ...
   struct size_helper< BiggestSupportedVectorExtension, MaximumLoadFactor, size_policy_hash::ARBITRARY > {
      ...
      size_helper(size_t const p_EstimatedDistinctCount) :
         ...
         m_Count{
            ( m_UnalignedCount + BiggestSupportedVectorExtension::vector_element_count() +
               ( ( - ( m_UnalignedCount & ( BiggestSupportedVectorExtension::vector_element_count() - 1 ) ) )
                  | ( - BiggestSupportedVectorExtension::vector_element_count() )
               )
            )
         }{}
   };
   ...
   struct index_resizer< VectorExtension, size_policy_hash::ARBITRARY > {
      ...
      struct state_t {
         ...
         typename VectorExtension::register_type const m_ResizeVector;
         state_t(size_t const p_Count ) :
            m_ResizeValue{ p_Count },
            m_ResizeVector{tsl::set1<VectorExtension>(p_Count)} {}
      };
      /**
   ...
   struct index_resizer< VectorExtension, size_policy_hash::EXPONENTIAL > {
      ...
      struct state_t {
         ...
         state_t(size_t const p_Count) :
            ...
            m_ResizeVector{ tsl::set1<VectorExtension>( m_ResizeValue ) } {}
      };
      /**
       * @brief Maps all keys from the vector register into the finite index space of the target hash based datastructure.
       * @param p_IndexValues Vector register which contains the indices to map.
      ...
      apply( typename VectorExtension::register_type const & p_IndexValues, state_t const & p_State ) {
         return
            tsl::binary_and<VectorExtension>(
               p_IndexValues,
               p_State.m_ResizeVector
            );
      }
   ...
   struct index_aligner {
      ...
      struct state_t {
         static_assert(
            std::is_same<
               typename VectorExtension::base_type,
         ...
         typename VectorExtension::register_type const m_AlignmentVector;
         state_t( void ) :
            m_AlignmentVector{
               tsl::set1<VectorExtension>(
                  (typename VectorExtension::base_type)~(BiggestSupportedVectorExtension::vector_element_count() - 1)
               )
            } {}
      };
      /**
      ...
      apply( typename VectorExtension::register_type const & p_IndexValues, state_t<BiggestSupportedVectorExtension> const & p_State ) {
         return tsl::binary_and<VectorExtension>( p_IndexValues, p_State.m_AlignmentVector );
      }
   };
